...
struct matrice* get_images(){
    struct matrice* train = matrice_new(SIZE, NUM_TRAIN);
    for(int i=0;i<NUM_TRAIN;i++){
        for (int j=0;j<SIZE;j++){
            matrice_set(train, j, i, train_image[i][j]);
        }
    }
    return train;
}

struct matrice* get_labels(){
    struct matrice* label = matrice_new(1,NUM_TRAIN);
    for(int i=0;i<NUM_TRAIN;i++){
        matrice_set(label, 0, i, train_label[i]);
    }
    return label;
}

struct matrice* get_test_images(){
    struct matrice* train = matrice_new(SIZE, NUM_TEST);
    for(int i=0;i<NUM_TEST;i++){
        for (int j=0;j<SIZE;j++){
            matrice_set(train, j, i, test_image[i][j]);
        }
    }
    return train;
}
struct matrice* get_test_labels(){
    struct matrice* label = matrice_new(1,NUM_TEST);
    for(int i=0;i<NUM_TEST;i++){
        matrice_set(label, 0, i, test_label[i]);
    }
    return label;
}
