...
int main(int argc, char** argv)
{
    ...
    SDL_Window* window = SDL_CreateWindow("Window", 0, 0,
            10,10,SDL_WINDOW_SHOWN | SDL_WINDOW_RESIZABLE);
    if (window == NULL)
        errx(EXIT_FAILURE, "%s", SDL_GetError());
    ...
    //Create a surface form the colored image.
    SDL_Surface* s = load_image(argv[1]);
    //SDL_Surface* s2 = load_image(argv[1]);
    ...
    //save_texture(renderer, texture, "resultat.png");
