...
{
    matrice *row = matrice_new(1, inputs->columns);
    for (int j = 0; j < inputs->columns; j++)
        matrice_set(row, 0, j, matrice_get(inputs, i, j));
    return row;
}

struct matrice* matrice_get_column(matrice* inputs, int i)
{
    matrice *column = matrice_new(inputs->rows, 1);
    for (int j = 0; j < inputs->rows; j++)
        matrice_set(column, j, 0, matrice_get(inputs, j, i));
    return column;
}
...
void matrice_sub_inplace(matrice *dest, matrice *source){
    if (dest->rows != source->rows || dest->columns != source->columns)
        errx(EXIT_FAILURE, "matrice_sub_inplace: incompatible matrice sizes, "
                            "dest is %d x %d, source is %d x %d",
                            dest->rows, dest->columns,
                            source->rows, source->columns);
